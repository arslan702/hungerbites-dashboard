...
const InactiveCustomersTab = ({ setInActive, inActive, setCurrentRowData, setOpenIn }) => {
    ...
    const url = resourceUrl(`auth/inactive`);
    ...
    const currentRows = inActive?.slice(start, end);
        ...
        <>
            ...
            >
                <TableBody className={classes.tableBody}>
                    {currentRows?.map((row) => {
                        const { id, updatedAt, name, UserAuthentication, phone_number, status } = row;
                        ...
                        return (
                            <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedRow} className={classes.root}>
                                ...
                                <TableCell
                                    align="left"
                                    sx={{ cursor: 'pointer', padding: '6px 16px 6px 16px' }}
                                ...
                                >
                                    <Label
                                        label={!UserAuthentication?.status ? 'active' : UserAuthentication?.status}
                                        sx={{
                                            color: 'white',
                                            backgroundColor: `${UserAuthentication?.status == 'active' ? '#1A9890' : '#9B2915'}`
                                        }}
                                    />
                                </TableCell>
            ...
            <Pagination
                ...
                count={Math.ceil(inActive?.length / rowsPerPage)}
                page={page}
...
export default InactiveCustomersTab;
