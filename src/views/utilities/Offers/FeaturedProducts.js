...
const FeaturedProducts = ({ value, setCurrentRowData, discountsList, setDiscountsList, setOpenEdit }) => {
    ...
    const handleDeleteClick = async () => {
        setOpen(null);
        await apiDelete('discount', { id: currentRowId });
        await fetchDiscounts();
    };
    ...
    const currentRows = discountsList?.slice(start, end);
    ...
    return (
        <>
                    ...
                    {currentRows?.map((row) => {
                        ...
                        return (
                            <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedRow} className={classes.root}>
                                ...
                                <TableCell align="left" sx={{padding: '6px 16px 6px 16px'}}>{MenuItem?.price}</TableCell>
                                <TableCell align="left" sx={{padding: '6px 16px 6px 16px'}}>{((discount_amount / MenuItem?.price) * 100).toFixed(2)} %</TableCell>
            ...
            <Pagination
                ...
                count={Math.ceil(discountsList?.length / rowsPerPage)}
                page={page}
